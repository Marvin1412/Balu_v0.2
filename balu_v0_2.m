...
nom_h = 0.1; %nominelle Höhe
nom_w = 0.15; %nominelle Breite
point_num = 3;%minpoint für DBSCAN
inner_wall = 1;
direction_down = 1;
travel_opti = 0;

alpha = 1; %Bastelfaktor
...
fill_area = 0;
agz_base = 5; %Zellgröße in mm 1.4 müsste 0.2mmm loch sein bis 0.1mm 
meshSize = 35; %
radius_base = 0.3;%radius für DBSCAN
point_density_factor = 15; % n points per mm^2

prev_z = 100000;

print_speed = 3000; %mm/min
travel_speed = 7500; %mm/min

retraction_speed = 2700; %mm/min
...
%Gyroid
gf = '@(x,y) sin(x.*2.*pi/agz).*cos(y.*2.*pi/agz)+sin(y.*2*pi/agz).*cos(2.*z_step.*pi/agz)+sin(2.*z_step.*pi/agz).*cos(2.*x.*pi/agz)';

%.gcode file to edit has to be input here
sgcode = 'C:\Users\marvi\Desktop\ThesisNeu\Coding\G-code Files to parse\Ausarbeitung\CE3_idd_v5_dt.gcode';
